"=============================================================================
" File:        ide.vim
" Author:      Daniel Gomez-Prado (dgomezpr at ecs dot umass dot edu)
" Last Change: March 19 2011 09:47:08 AM EST
" Version:     0.4.3
...
if !exists('g:IDE_isLoaded')
	...
	"let s:IDE_Divider = s:IDE_useDigraphs ? "■■■■" : "%%%%"
	"let s:ide = s:IDE_useDigraphs ? "ⅰ∂ε" : "ide"
	let s:divw = 5
	while ( s:divw > 0 )
...
function! s:IDE_DisplayHelp()
	...
	if s:IDE_isBriefHelp
		...
		"call s:AppendHelp('+',': Open a fold')
		"call s:AppendHelp('-',': Close a fold')
		"call s:AppendHelp('*',': Open all folds')
		"call s:AppendHelp('=',': Close all folds')
...
function! s:IDE_InitializeCommands()
	...
	if s:IDE_isSyntaxCapable != 0
		command! -nargs=0 -bar IDESyntax call s:IDE_UpdateSyntaxFile()
	endif
	">>>
...
function! s:IDE_CreateSyntaxTags(verbose)
	...
	let l:tag_arg = ' --c++-kinds=+p --fields=+aKz --extra= --languages=c++ --langmap=C++:+.inl -f '.s:IDE_ProjectSyn['tags']
...
function! s:IDE_UpdateSyntaxFile()
	if bufnr('%') == s:IDE_Buffer
...
function! s:IDE_StartSyntax()
	...
	if filereadable(s:IDE_ProjectSyn['syntax'])
		...
		"call s:IDE_InfoMsg("Found IDE syntax for ".s:IDE_ProjectName."(".s:IDE_ProjectSyn['syntax'].")")
